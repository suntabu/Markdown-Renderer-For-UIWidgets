...
namespace markdown
{
    public class InlineParser
    {
        ...
        public InlineParser(string source, Document document)
        {
            ...
            _stack = new List<TagState>();
            // User specified syntaxes are the first syntaxes to be evaluated.
            syntaxes.AddRange(document.inlineSyntaxes);
            ...
            if (documentHasCustomInlineSyntaxes)
            {
                ...
                syntaxes.Add(new TextSyntax(@"[A-Za-z0-9]+(?=\s)"));
            }
            else
            {
                syntaxes.Add(new TextSyntax(@"[ \tA-Za-z0-9]*[A-Za-z0-9](?=\s)"));
            }

            syntaxes.AddRange(_defaultSyntaxes);

            if (this.document.encodeHtml)
            {
                syntaxes.AddRange(_htmlSyntaxes);
            }

            // Custom link resolvers go after the generic text syntax.
            syntaxes.InsertRange(1, new List<InlineSyntax>()
            {
...
//                    Debug.Log("continue");
        ...
        internal int charAt(int index) => source.codeUnitAt(index);
...
//            Debug.LogError(source+"===>" + text + "   " + _stack.Count);
        ...
        internal void openTag(TagState state) => _stack.Add(state);

        internal bool isDone => pos == source.Length;
    ...
    public abstract class InlineSyntax
    {
        ...
        protected InlineSyntax(string pattern)
        {
        ...
        {
            if (startMatchPos == -1) startMatchPos = parser.pos;

//            Debug.Log("===>  " + pattern.ToString() + "  -> " + parser.source + "  " + startMatchPos);

            var startMatch = pattern.matchAsPrefix(parser.source, startMatchPos);
//            if (startMatch.Success)
//            {
//                Debug.Log("===>  " + pattern + " -> " + startMatch.Groups[0].Value + "  " + startMatchPos + "   " +
//                          parser.source);
//            }
//            else
//            {
//                Debug.LogWarning("===>  " + pattern + " -> " + startMatch.Groups[0].Value + "  " + startMatchPos + "   " +
//                parser.source);
//            }
...
// https://github.github.com/gfm/#example-600
            if (url.endsWith(')'))
            {
                int opening = _countChars(url, '(');
                int closing = _countChars(url, ')');
                if (closing > opening)
                {
...
// Could not parse a delimiter run.
        ...
        String toString() => string.Format("<char: {0}, length: {1}, isLeftFlanking: {2}, isRightFlanking: {3}>", cha,
            length, isLeftFlanking, isRightFlanking);

// Whether a delimiter in this run can open emphasis or strong emphasis.
        internal bool canOpen =>
            isLeftFlanking && (cha == CharCode.asterisk || !isRightFlanking || isPrecededByPunctuation);

// Whether a delimiter in this run can close emphasis or strong emphasis.
        internal bool canClose =>
            isRightFlanking && (cha == CharCode.asterisk || !isLeftFlanking || isFollowedByPunctuation);
    }
...
// brackets, so that, e.g. a link resolver can receive `[\[\]]` as `[]`.
                return linkResolver(label.replaceAll(@"\\", @"\").replaceAll(@"\[", '[').replaceAll(@"\]", ']'));
            }
        }
...
// Advance past the closing delimiter.
            ...
            _moveThroughWhitespace(parser);
            if (parser.isDone) return null;
            if (parser.charAt(parser.pos) != CharCode.rparen) return null;
            return title;
        }
    }
    ...
    class CodeSyntax : InlineSyntax
    {
        ...
        internal override bool tryMatch(InlineParser parser, int startMatchPos)
        {
            ...
            parser.writeText();
            if (onMatch(parser, match)) parser.consume(match.Groups[0].Length);
            return true;
        }
    ...
    class TagState
    {
        ...
        internal List<Node> close(InlineParser parser, Match endMatch)
        {
            ...
            foreach (var unmatched in unmatchedTags)
            {
                ...
                children.AddRange(unmatched.children);
            }
            ...
            else
            {
                ...
                parser._stack.last().children.AddRange(children);
        ...
        internal String textContent =>
            children.Select((Node child) => child.textContent).join("");
    }
